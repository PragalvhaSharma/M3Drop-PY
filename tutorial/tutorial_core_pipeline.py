...
if __name__ == "__main__":
    ...
    if not os.path.exists(FIT_OUTPUT_FILE):
        fit_results = M3Drop.NBumiFitModel(
    ...
    if not os.path.exists(HIGH_VAR_OUTPUT_CSV):
        high_var_genes = M3Drop.NBumiFeatureSelectionHighVar(fit=fit_results)
    ...
    if not os.path.exists(COMBINED_DROP_OUTPUT_CSV):
        combined_drop_genes = M3Drop.NBumiFeatureSelectionCombinedDrop(
            fit=fit_results,
            cleaned_filename=CLEANED_DATA_FILE
        )
    ...
    if not os.path.exists(VOLCANO_PLOT_FILE):
        ...
        M3Drop.NBumiCombinedDropVolcano(
