try:
    # Attempt relative import (Works for PyPI / Installed Package)
    from .coreGPU import get_optimal_chunk_size
except ImportError:
    # Fallback to absolute import (Works for Local Testing / Direct Run)
    from coreGPU import get_optimal_chunk_size
    ...
    with h5py.File(output_filename, 'a') as f_out:
        ...
        with h5py.File(cleaned_filename, 'r') as f_in:
            ...
            for i in range(0, nc, chunk_size):
                ...
                del counts_chunk_dense_gpu, mus_chunk_gpu, pearson_chunk_gpu, denominator_gpu
                del counts_chunk_sparse_gpu
...
def NBumiPearsonResidualsApproxGPU(
    ...
    """
    Calculates approximate Pearson residuals. Safe Mode: Multiplier increased to 5.0.
    """
    start_time = time.perf_counter()
    print(f"FUNCTION: NBumiPearsonResidualsApprox() | FILE: {cleaned_filename}")

    # --- SAFETY UPDATE ---
    # Multiplier 5.0 (Was 3.5): Safer chunk sizes.
